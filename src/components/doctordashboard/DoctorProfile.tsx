...
import EditProfileModal from './doctorsProfile/EditProfileModal';
import ChangePasswordModal from './doctorsProfile/ChangePasswordModal';
import ProfileDetails from './doctorsProfile/ProfileDetails';
...
const DoctorProfile = () => {
  ...
  const displayProfilePicture =
    ...
    `https://ui-avatars.com/api/?name=${encodeURIComponent(userDetails?.firstName || user?.firstName || '')}&background=0ea5e9&color=fff&size=128`;
        ...
        <div className="max-w-4xl mx-auto rounded-lg shadow-lg p-5">
          <div className="flex flex-col md:flex-row items-center justify-between border-b border-gray-700 pb-5 mb-5">
            <div className="relative flex items-center gap-4 mb-4 md:mb-0">
              ...
              />
              <label className="absolute bottom-0 bg-sky-500 p-2 rounded-full cursor-pointer">
                <FaCamera />
            ...
            <button className="btn bg-sky-600 flex items-center gap-2" onClick={() => setIsProfileModalOpen(true)}>
              <FaEdit /> Edit Profile
            </button>
          </div>
